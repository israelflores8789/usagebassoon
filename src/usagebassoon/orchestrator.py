...
from usagebassoon.backends.base import SourceLeaseBusy, StorageBackend, close_backend
...
from usagebassoon.source_leases import ActiveSourceLease, source_lease
...
def _collect_locked(
    ...
    started_at: datetime,
    logger: logging.Logger,
    lease: ActiveSourceLease,
) -> PersistSummary:
    """Collect and persist while the logical source remains leased."""
    ...
    requested_price_days = tuple(
        day
        for day in candidate_days
        if day == today or day in daily_days or (day, _PRICING_DOMAIN) not in completed
    )
    ...
    lease.check()
    return persist_with_retries(config, normalize(bundle), logger, lease=lease.token)
...
def collect(config: UsageBassoonConfig) -> tuple[str, PersistSummary]:
    ...
    run_id = str(uuid4())
    try:
        with source_lease(config, run_id, logger) as lease:
            summary = _collect_locked(config, run_id, datetime.now(UTC), logger, lease)
